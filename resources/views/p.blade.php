        ...
        @section('nav-link')
            ...
            <div class="second_side">
                ...
                <div class="profile-picture">
                    <div class="pp-cnt">
                        <img onclick="showPhoto()" class="profile-photo"
                            src="{{ $user->user->profile->pphoto ? asset('storage/' . $user->user->profile->pphoto) : asset('images/default.jpeg') }}"
                            alt="">
                ...
                <div class="rest">
                    <div class="profile-details">
                        ...
                        <p>{{ $user->bio }}</p>
                        <p>{{ $user->email }}</p>
                        <div class="joined"><img class="calender" src="/icon/calendar3.svg" alt="">
                            <p class="whitesmoke">{{ 'Joined ' . $user->user->created_at }}</p>
                        </div>
                        <div class="followers">
                            <p class="whitesmoke"><span
                                    style="color:black; font-weight:bold;">{{ $user->user->following->count() }}
                                </span>Following</p>
                            <p class="whitesmoke"><span
                                    style="color:black; font-weight:bold;">{{ $user->user->followers->count() }}
                                </span>Followers</p>
                        </div>
                    </div>
                    <div class="sections">
                        <div class="sec-cnt">
                            ...
                            <h4 class="tablinks" onclick="slide(event, 'tweetAndReply')">Tweets & replies
                                ({{ count($comments) }})</h4>
                    ...
                    <div class="tweetss" id="tweet">
                        @unless (count($tweets) == 0)
                                    ...
                                    <div class="info">
                                        <div class="prof">
                                            ...
                                            <div class="id">
                                                ...
                                                <p style="font-size:smaller;">{{ '@' . $user->user->username }}</p>
                            ...
                            @endforeach
                        @else
                            <h1 style="display: flex; justify-content:center; align-items:center; margin-top:20px;">No tweets
                                found</h1>
                        @endunless
                    </div>
                    ...
                    <div class="tweetss" id="tweetAndReply" style="display: none;">
                        @unless (count($comments) == 0)
                            @foreach ($comments as $comment)
                                ...
                                <div id="id{{ $id }}" class="view-tweet">
                                    <div class="info">
                                        <div class="prof">
                                            <img class="p-photo"
                                                src="{{ $comment->user->profile->pphoto ? asset('storage/' . $comment->user->profile->pphoto) : asset('images/default.jpeg') }}"
                                                alt="">
                                            <div class="id">
                                                <h3>{{ $comment->user->name }}</h3>
                                                <p style="font-size: large;">{{ '@' . $comment->user->username }}</p>
                                                <h5>{{ $comment->created_at }}</h5>
                                                <img onclick="menue('id{{ $id }}')" class="menue"
                                                    src="icon/three-dots.svg" alt="">
                                            </div>
                            ...
                            @endforeach
                        @else
                            <h1 style="display: flex; justify-content:center; align-items:center; margin-top:20px;">No Comments
                                made</h1>
                        @endunless
                    </div>
                    {{-- End... --}}
            ...
            <div class="third-container">
                <div class="th-container">
                    <div class="search-head">
                        ...
                        <form action="#">
                            <input class="search" type="search" name="profile-search" id=""
                                placeholder="Search {{ $user->user->username }} pfofile">
                        </form>
                    </div>
...
<div class="photo-display">
    <p style="cursor: pointer; font-size: 30px; width: 3%; margin-left: 90%; margin-bottom:6%" onclick="exitPhoto()">
        &times</p>
    <img class="big-photo"
        src="{{ $user->user->profile->pphoto ? asset('storage/' . $user->user->profile->pphoto) : asset('images/default.jpeg') }}"
        alt="">
